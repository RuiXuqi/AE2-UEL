...
public class GuiSecurityStation extends GuiMEMonitorable {
    ...
    public void initGui() {
        ...
        this.inject = newToggleButtonToList(this.guiLeft + 56, top, 11 * 16, 12 * 16, SecurityPermissions.INJECT
                .getUnlocalizedName(), SecurityPermissions.INJECT.getUnlocalizedTip());

        this.extract = newToggleButtonToList(this.guiLeft + 56 + 18, top, 11 * 16 + 1, 12 * 16 + 1, SecurityPermissions.EXTRACT
                .getUnlocalizedName(), SecurityPermissions.EXTRACT.getUnlocalizedTip());

        this.craft = newToggleButtonToList(this.guiLeft + 56 + 18 * 2, top, 11 * 16 + 2, 12 * 16 + 2, SecurityPermissions.CRAFT
                .getUnlocalizedName(), SecurityPermissions.CRAFT.getUnlocalizedTip());

        this.build = newToggleButtonToList(this.guiLeft + 56 + 18 * 3, top, 11 * 16 + 3, 12 * 16 + 3, SecurityPermissions.BUILD
                .getUnlocalizedName(), SecurityPermissions.BUILD.getUnlocalizedTip());

        this.security = newToggleButtonToList(this.guiLeft + 56 + 18 * 4, top, 11 * 16 + 4, 12 * 16 + 4, SecurityPermissions.SECURITY
